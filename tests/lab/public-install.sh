...
omarchy_host_test() {
  ...
  wait_for_guest_state "public clean clone is installed, enabled, and loaded at the expected commit" 30 ssh_session \
    "test -d \"$plugin_dir/.git\" && \
     ...
     omarchy-shell disk-lens-service state | jq -e '.buildIdentity == \"disk-lens-service-v0602\"' && \
     omarchy-shell disk-lens state | jq -e '.buildIdentity == \"disk-lens-widget-v0602\"'" || {
