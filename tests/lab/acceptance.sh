...
omarchy_host_test() {
  ...
  local scan_button_x scan_button_y view_button_x search_x filter_y clear_x
  local scanned_before long_scope
  ...
  wait_for_guest_state "service and widget load with matching candidate identity" 25 ssh_session \
    "omarchy-plugin-list --json | jq -e 'any(.[]; .id == \"io.github.mtolhuys.disk-lens\" and .enabled == true)' && \
     omarchy-shell disk-lens-service state | jq -e '.buildIdentity == \"disk-lens-service-v0100\" and (.capacityState == \"ready\" or .capacityState == \"loading\")' && \
     omarchy-shell disk-lens state | jq -e '.buildIdentity == \"disk-lens-widget-v0100\" and .opened == false'" || {
  ...
  qmp_pointer_tap "$screen_width" "$screen_height" $((screen_width - 475)) 450 left
  ...
  capture_console "success-disk-lens-03-filtered-list"
  ...
  capture_console "success-disk-lens-04-light-theme"
  ...
  capture_console "success-disk-lens-05-cancelled"
  ...
  capture_console "success-disk-lens-06-partial"
  ...
  capture_console "success-disk-lens-07-long-scope"
  ...
  capture_console "success-disk-lens-08-empty"
  ...
  ssh_guest "sed -i 's/disk-lens-service-v0100/disk-lens-service-v0100-labupdate/' \
      /home/omarchy/.cache/omarchy-disk-lens/development-source/src/Service.qml && \
    sed -i 's/disk-lens-widget-v0100/disk-lens-widget-v0100-labupdate/' \
  ...
  wait_for_guest_state "same-path update replaces service and widget runtime identities" 30 ssh_session \
    "omarchy-shell disk-lens-service state | jq -e '.buildIdentity == \"disk-lens-service-v0100-labupdate\"' && \
     omarchy-shell disk-lens state | jq -e '.buildIdentity == \"disk-lens-widget-v0100-labupdate\"'" || {
  ...
  wait_for_guest_state "re-enable restores one service and one widget" 25 ssh_session \
    "omarchy-shell disk-lens-service state | jq -e '.buildIdentity == \"disk-lens-service-v0100-labupdate\"' && \
     omarchy-shell disk-lens state | jq -e '.buildIdentity == \"disk-lens-widget-v0100-labupdate\"'" || return 1
  ...
  capture_console "success-disk-lens-09-removed"

  printf 'ok - Disk Lens pointer, themes, scan states, filters, runtime update, lifecycle, and cleanup passed\n'
}
