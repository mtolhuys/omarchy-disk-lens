...
omarchy_host_test() {
  ...
  wait_for_guest_state "service and widget load with matching candidate identity" 25 ssh_session \
    "omarchy-plugin-list --json | jq -e 'any(.[]; .id == \"io.github.mtolhuys.disk-lens\" and .enabled == true)' && \
     omarchy-shell disk-lens-service state | jq -e '.buildIdentity == \"disk-lens-service-v0600\" and (.capacityState == \"ready\" or .capacityState == \"loading\")' && \
     omarchy-shell disk-lens state | jq -e '.buildIdentity == \"disk-lens-widget-v0600\" and .opened == false'" || {
  ...
  ssh_guest "sed -i 's/disk-lens-service-v0600/disk-lens-service-v0600-labupdate/' \
      /home/omarchy/.cache/omarchy-disk-lens/development-source/src/Service.qml && \
    sed -i 's/disk-lens-widget-v0600/disk-lens-widget-v0600-labupdate/' \
  ...
  wait_for_guest_state "same-path update replaces service and widget runtime identities" 30 ssh_session \
    "omarchy-shell disk-lens-service state | jq -e '.buildIdentity == \"disk-lens-service-v0600-labupdate\"' && \
     omarchy-shell disk-lens state | jq -e '.buildIdentity == \"disk-lens-widget-v0600-labupdate\"'" || {
  ...
  wait_for_guest_state "re-enable restores one service and one widget" 25 ssh_session \
    "omarchy-shell disk-lens-service state | jq -e '.buildIdentity == \"disk-lens-service-v0600-labupdate\"' && \
     omarchy-shell disk-lens state | jq -e '.buildIdentity == \"disk-lens-widget-v0600-labupdate\"'" || return 1
