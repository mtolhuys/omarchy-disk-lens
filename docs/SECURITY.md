...
- Each helper forwards `TERM` to its owned `du` or `find` child, waits for it, removes its private temporary directory, and exits `130`.
...
Version `0.5.0` keeps capacity, bounded navigation snapshots, folder-browser state, filters, selection, and agent-dispatch bookkeeping in memory. It writes no scan-result cache or plugin settings. Disablement and removal unload the service and widget; scanned user files stay untouched. The opt-in development installer uses a dedicated XDG-cache snapshot and replaces only plugin id `io.github.mtolhuys.disk-lens`.
