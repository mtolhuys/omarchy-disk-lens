...
**Decision:** While the owned scan job is active, show one restrained rotating ring on the bar gauge and one soft radar sweep in the results pane, duplicated in literal text, controls, and accessibility metadata.
...
**Consequence:** Motion stops immediately when scanning completes, fails, or is cancelled. The radar is hero-sized on first measurement and compact when refreshing an existing result. Composed reduced-motion behavior remains a release gate.
