...
**Consequence:** Motion stops immediately when scanning completes, fails, or is cancelled. The vertical stripe fills the hero pane on first measurement and overlays the live list/treemap when refreshing an existing result. Composed reduced-motion behavior remains a release gate.
