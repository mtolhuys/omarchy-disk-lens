...
Authoritative product, UX, architecture, security, dependency, test, release, contribution, support, and license contracts. English-only documentation and repository checks pass in `make test`.

## M1 — Honest native vertical slice — complete

Valid Omarchy service and bar-widget entry points; theme-native pie gauge, capacity rail, and panel; strict capacity and scan adapters; synthetic immediate-child scans; and loaded build identities.
...
The complete visual analysis journey stays inside Disk Lens. Secondary analyzer detection, package-install surfaces, launch actions, lifecycle branches, tests, and marketing were removed. The resulting panel is smaller and has one unambiguous next-step model.

## M5 — Release hardening — in progress

Completed work includes exact Plugin Lab evidence, same-path runtime updates, full enable/disable/re-enable/remove lifecycle, exact-working-tree development installation, synthetic screenshots, light/dark review, bar activity ring plus vertical scanner stripe with literal status equivalents, deterministic README media, strict shallow-folder and scan protocols, cache-restored navigation, bounded scanner post-processing, and exact selected-item moves through a guarded desktop Trash boundary.
...
These are not version `0.5.2` promises.
