...
5. The user explicitly starts or refreshes an immediate-child scan; a bounded bar activity ring plus a smooth vertical scanner stripe and status text stay visible until that job ends.
...
- One purposeful live activity ring in the bar and a soft vertical scanner stripe in the results pane, without replacing the last completed result.
...
- Safe native actions: Back, choose folder, drill via Enter/→, open in file manager, refresh, cancel, clear filters, Ask Omarchy, and a confirmed move to desktop Trash.
